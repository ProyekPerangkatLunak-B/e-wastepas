<div>
    <label for="{{ $id }}"
        class="block mt-2 text-sm font-medium leading-7 text-gray-500">{{ $label }}</label>
    <div class="relative mt-2">
        <input id="{{ $id }}" name="{{ $name }}" type="{{ $type }}"
            placeholder="{{ $placeholder }}" required autofocus value="{{ old($name) }}"
        ...
        @if ($type === 'password')
            <button type="button" onclick="togglePassword('{{ $id }}')"
                ...
                <svg id="eye-open-{{ $id }}" class="hidden w-5 h-5 text-gray-500" viewBox="0 0 24 24"
                    fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"
                        fill="currentColor" />
                </svg>
                <svg id="eye-closed-{{ $id }}" class="w-5 h-5 text-gray-500"
