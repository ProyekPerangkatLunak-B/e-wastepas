...
@section('content')
        ...
        <div class="py-8">
            ...
            <div class="px-12 mt-6">
                ...
                <div class="overflow-x-auto bg-white rounded-lg shadow-md">
                    <table id="kurirTable" class="w-full border border-gray-300 bg-white rounded-lg">
    ...
    <script>
        ...
        function changePage(page) {
            $('#kurirTable').DataTable().page(page).draw('page');
        }

        document.addEventListener('DOMContentLoaded', () => {
            $(document).ready(function() {
                var table = $('#kurirTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: function(data, callback, settings) {
                        ...
                        $.ajax({
                            url: '{{ route('admin.datamaster.kurir.getData') }}',
                            method: 'GET',
                            data: {
        ...
        function approveVerification(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/kurir/${id}/approve`, // Endpoint approve
                        type: 'PATCH',
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            $('#kurirTable').DataTable().ajax.reload(); // Reload DataTable
                        },
                        error: function(xhr) {
                            Swal.fire(
                                'Gagal!',
        ...
        function rejectVerification(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/kurir/${id}/reject`, // Endpoint reject
                        type: 'PATCH',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('admin.datamaster.kurir.destroy', '') }}/${id}`,
                        type: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        function fetchDokumen(userId) {
            $.ajax({
                ...
                `;
                    ...
                    $('#documentModal').removeClass('hidden'); // Show modal
                },
                error: function(xhr) {
        ...
        function closeDocumentModal() {
            $('#documentModal').addClass('hidden'); // Hide modal
        }
    </script>
@endsection
