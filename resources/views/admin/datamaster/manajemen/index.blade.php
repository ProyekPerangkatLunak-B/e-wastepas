...
@section('content')
    ...
    <div class="container max-w-full px-4 mx-auto bg-gray-50">
        <div class="py-8">
            <h2 class="text-2xl font-bold leading-relaxed ml-14 text-green-500">Dashboard Manajemen</h2>
            <h4 class="text-base font-light ml-14 text-gray-600">Selamat datang di dashboard manajemen.</h4>
    ...
    <script>
        ...
        function changePage(page) {
            $('#manajemenTable').DataTable().page(page).draw('page');
        }

        document.addEventListener('DOMContentLoaded', () => {
            $(document).ready(function() {
                var table = $('#manajemenTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: function(data, callback, settings) {
                        ...
                        $.ajax({
                            url: '{{ route('admin.datamaster.manajemen.getData') }}',
                            method: 'GET',
                            data: {
            ...
            `;
                ...
                <div class="flex flex-col items-center space-y-2" style="color: white">
                    <a href="/admin/manajemen/detail/${data}"
        ...
        function approveVerification(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/manajemen/${id}/approve`, // Endpoint approve
                        type: 'PATCH',
                        headers: {
                        ...
                        success: function(response) {
                            ...
                            $('#manajemenTable').DataTable().ajax.reload(); // Reload DataTable
                        },
                        error: function(xhr) {
                            Swal.fire(
                                'Gagal!',
        ...
        function rejectVerification(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/admin/manajemen/${id}/reject`, // Endpoint reject
                        type: 'PATCH',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('admin.datamaster.manajemen.destroy', '') }}/${id}`,
                        type: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        success: function(response) {
                            ...
                            $('#manajemenTable').DataTable().ajax.reload();
                        },
                        error: function(xhr) {
                            Swal.fire(
                                'Gagal!',
